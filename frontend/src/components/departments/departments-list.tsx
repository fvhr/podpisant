...
export const DepartmentsList: React.FC = () => {
    ...
    return (
        <div className="access-control">
            <h1 className="access-control__title">Управление доступом</h1>

            <div className="access-control__list">
                {departments.map((department) => (
                    <div
                        key={department.id}
                        className={`access-control__department ${expandedStage === department.id ? 'expanded' : ''}`}
                    >
                        <div
                            className="access-control__header"
                            onClick={() => toggleStage(department.id)}
                        >
